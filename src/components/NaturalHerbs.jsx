...
function NaturalHerbs() {
  const data = useContext(BaharatContext);
  const navigate = useNavigate();

  const filteredData = data.find((item) => item.category === 'Təbii otlar');
    ...
    <>
      <div style={{ background: '#eee' }}>
        <div className="haqqimizdaBg">
          <h2 className="mehsul-haqqinda">Təbii otlar</h2>
        </div>
      </div>
